module Billing
  class Notification::VoidPayment < Struct.new(:options)
    
    def perform
      if _transaction = ::Transaction.find_by_uuid(billing_transaction.uuid)
        _transaction.update_attributes(state: billing_transaction.status.downcase)
      else
        ::Transaction.create({
